...
async function regenerateAllPNGs() {
  initDatabase();
  const pool = getPool();
  
  console.log('\n🎨 Regenerating ALL PNGs with strokeWidth 1.0\n');
  ...
  try {
    ...
    for (let i = 0; i < result.rows.length; i++) {
      ...
      try {
        const sigilPNG = await canvasToPNG(row.sigil_code, {
          ...
          canvasWidth: 100,
          canvasHeight: 100,
          strokeWidth: 1.0,  // New thinner weight
          scale: 0.75
        });
    ...
    console.log(`Summary:`);
    console.log(`   - ${successCount} PNGs regenerated (strokeWidth: 1.0)`);
